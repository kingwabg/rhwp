...
## v2a 증분 (2026-07-30, 사용자 실사용 신고 후)
- 신고: 삭제 취소선이 안 남음 + 한컴과 다른 표시. 재현 결과 **선택(범위) 삭제와 표 셀
  편집이 훅 밖**이라 진짜 지워지고 있었다(사용자 문서가 표 중심 — 정확히 그 구멍).
- 수리: delete_range_native·insert/delete_text_in_cell_native 훅 + 셀 마크 열거/검토/
  사이드카(cell 좌표) + studio deleteSelection 스냅샷 승격 + 셀 rect 오버레이.
- 한컴식 표시로 개편: 삽입=빨간 밑줄(#e03131)+옅은 붉은 배경, 삭제=빨간 취소선,
  왼쪽 여백 빨간 변경 막대(구간 병합). '본 최종' 보기 = 스냅샷 저장→모두 적용→읽기 전용
  미리보기→복원(리본 검토 탭 '본 최종' 토글).
- 판정: Rust 3건(범위·셀·사이드카 왕복 포함) + e2e 8단계(선택삭제 무손실·셀 추적·
  변경 막대·빨강 rgb(224,49,49)·본 최종 잠금/복원). 남은 한계: 머리말·각주 미추적,
  범위 삭제로 안 걸치는 문단 병합, 한컴 표준 태그(v2b), 셀 다중 문단 범위 삭제의
  중간 문단(전체 마크만).

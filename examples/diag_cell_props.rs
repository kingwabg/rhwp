...
fn main() {
    ...
    println!("set_cell_properties(w15000,h2000) → {}", r.map(|_| "OK".to_string()).unwrap_or_else(|e| format!("{e:?}")));
    ...
    println!("set_cell_properties(borderFillId) → {}", r.map(|_| "OK".to_string()).unwrap_or_else(|e| format!("{e:?}")));
    show(&doc, "after border");
    let r = doc.resize_table_cells(0, pi as u32, ci as u32, r#"[{"cellIdx":0,"widthDelta":1000,"localResize":true}]"#);
    println!("resize(local +1000) → {}", r.map(|_| "OK".to_string()).unwrap_or_else(|e| format!("{e:?}")));

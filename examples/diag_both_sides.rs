...
fn main() {
    ...
    let filler = |n: usize| {
        format!("채움 {n:02} 가나다라마바사 아자차카타파하 강물이 흐르고 산이 높다 바다는 깊고")
    };
    ...
    fn walk(
        n: &RenderNode,
        tbl: &mut (f64, f64, f64, f64),
        out: &mut Vec<(i32, i32, i32, String)>,
    ) {
    ...
    for (y, x, w, t) in lines
        .iter()
        .filter(|l| (l.0 as f64) > tbl.1 - 60.0 && (l.0 as f64) < tbl.1 + tbl.3 + 40.0)
    {

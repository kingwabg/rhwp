...
fn main() {
    ...
    let c: serde_json::Value =
        serde_json::from_str(&doc.create_table(0, 0, 50, 3, 3).unwrap()).unwrap();
    let (pi, ci) = (
        c["paraIdx"].as_u64().unwrap() as u32,
        c["controlIdx"].as_u64().unwrap() as u32,
    );
    let rb = |d: &HwpDocument| -> String {
        serde_json::from_str::<serde_json::Value>(&d.get_table_properties(0, pi, ci).unwrap())
            .unwrap()["textWrap"]
            .as_str()
            .unwrap()
            .to_string()
    ...
    for wrap in ["Square", "Tight", "Through", "BehindText"] {
        doc.set_table_properties(0, pi, ci, &format!(r#"{{"textWrap":"{wrap}"}}"#))
            .unwrap();

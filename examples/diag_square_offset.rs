...
fn main() {
    ...
    for wrap in ["Square", "Tight", "TopAndBottom"] {
        ...
        let y0: serde_json::Value =
            serde_json::from_str(&doc.get_table_bbox(0, pi, ci).unwrap()).unwrap();
        doc.move_table_offset(0, pi, ci, 0, (100.0 * mm).round() as i32)
            .unwrap();
        let y1: serde_json::Value =
            serde_json::from_str(&doc.get_table_bbox(0, pi, ci).unwrap()).unwrap();
        println!(
            "{wrap:12} y0={:7.1} +100mm→ y1={:7.1} (이동 {:6.1}px)",
            y0["y"].as_f64().unwrap(),
            y1["y"].as_f64().unwrap(),
            y1["y"].as_f64().unwrap() - y0["y"].as_f64().unwrap()
        );
    }
}

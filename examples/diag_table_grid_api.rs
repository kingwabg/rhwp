...
fn main() {
    ...
    println!(
        "range cell0 right = {}",
        doc.get_boundary_move_range(0, pi, ci, 0, "right").unwrap()
    );
    println!(
        "range cell0 bottom = {}",
        doc.get_boundary_move_range(0, pi, ci, 0, "bottom").unwrap()
    );
    doc.offset_cell_boundary_native(0, pi as usize, ci as usize, 0, true, 1000)
        .unwrap();
    let g: serde_json::Value =
        serde_json::from_str(&doc.get_table_grid(0, pi, ci).unwrap()).unwrap();
    println!("after stagger cell0 right +1000:");
    println!(
        "  colX={} rowYEff={} rowCount={} colCount={} minCell={}",
        g["colX"], g["rowYEff"], g["rowCount"], g["colCount"], g["minCell"]
    );

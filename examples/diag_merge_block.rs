...
fn dump(doc: &HwpDocument, pi: usize, ci: usize, tag: &str) {
    ...
    println!("--- {tag}: common={}x{} raw={:?} rows={:?} eff={:?}", t.common.width, t.common.height,
        t.get_raw_row_heights(), t.get_row_heights(), t.effective_row_heights());
    for (i, c) in t.cells.iter().enumerate() {
        println!("  #{i} ({},{}) span {}x{} h={} w={} paras={}", c.row, c.col, c.row_span, c.col_span, c.height, c.width, c.paragraphs.len());
    }
}
fn main() {

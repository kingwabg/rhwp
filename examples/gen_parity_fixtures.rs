...
fn main() {
    ...
    {
        let mut doc = base();
        doc.insert_text(0, 0, 0, "앞 본문입니다. 표는 글자처럼취급으로 이 뒤에 붙습니다.").unwrap();
        ...
        for r in 0..3u32 { for col in 0..3u32 {
            doc.insert_text_in_cell(0, pi, ci, r * 3 + col, 0, 0, &format!("{}{}", ["가","나","다"][r as usize], col + 1)).unwrap();
        }}
    ...
    // F02: tac 표 쪽 경계 — 본문으로 밀어 표가 쪽 끝에 걸치게 (원자성 축 §3.5)
    {
        ...
        let (pi, ci) = (c["paraIdx"].as_u64().unwrap() as u32, c["controlIdx"].as_u64().unwrap() as u32);
        for r in 0..6u32 { for col in 0..2u32 {
            doc.insert_text_in_cell(0, pi, ci, r * 2 + col, 0, 0, &format!("행{}열{}", r + 1, col + 1)).unwrap();
        }}
    ...
    // F03: 셀 서식 — fontSize·bold·textColor·정렬 (공문 축)
    {
        ...
        let (pi, ci) = (c["paraIdx"].as_u64().unwrap() as u32, c["controlIdx"].as_u64().unwrap() as u32);
        doc.insert_text_in_cell(0, pi, ci, 0, 0, 0, "크게 20pt").unwrap();
        doc.apply_char_format_in_cell(0, pi as usize, ci as usize, 0, 0, 0, 999, r#"{"fontSize":20}"#).unwrap();
        doc.insert_text_in_cell(0, pi, ci, 1, 0, 0, "굵게").unwrap();
        doc.apply_char_format_in_cell(0, pi as usize, ci as usize, 1, 0, 0, 999, r#"{"bold":true}"#).unwrap();
        doc.insert_text_in_cell(0, pi, ci, 2, 0, 0, "빨강").unwrap();
        doc.apply_char_format_in_cell(0, pi as usize, ci as usize, 2, 0, 0, 999, r##"{"textColor":"#FF0000"}"##).unwrap();
        doc.insert_text_in_cell(0, pi, ci, 3, 0, 0, "가운데").unwrap();
        doc.apply_para_format_in_cell(0, pi as usize, ci as usize, 3, 0, r#"{"alignment":"center"}"#).unwrap();
    ...
    // F04: 열폭 + 병합 (일지 축) — 1행 전체 병합 헤더 + 2:1 열폭
    {
        ...
        doc.insert_text_in_cell(0, pi, ci, 0, 0, 0, "병합 헤더").unwrap();
        doc.insert_text_in_cell(0, pi, ci, 2, 0, 0, "넓은 열(2/3)").unwrap();
        doc.insert_text_in_cell(0, pi, ci, 3, 0, 0, "좁은 열(1/3)").unwrap();
    ...
    // F05: 누름틀 필드 (일지 치환 축)
    {
        ...
        let len = doc.get_paragraph_length(0, 0).unwrap();
        doc.insert_click_here_field_api(0, 0, len, "이름을 입력하세요", "", "name", true).unwrap();

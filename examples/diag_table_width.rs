...
fn dump(doc: &HwpDocument, pi: u32, ci: u32, tag: &str) {
    ...
    let c0: serde_json::Value =
        serde_json::from_str(&doc.get_cell_properties(0, pi, ci, 0).unwrap()).unwrap();
    let c1: serde_json::Value =
        serde_json::from_str(&doc.get_cell_properties(0, pi, ci, 1).unwrap()).unwrap();
    println!(
        "{tag:28} tableWidth={:6} bboxW={:6.1} cell0={:6} cell1={:6}",
        p["tableWidth"],
...
fn main() {
    ...
    let _ = doc.resize_table_cells(
        0,
        pi,
        ci,
        r#"[{"cellIdx":0,"widthDelta":-8000},{"cellIdx":1,"widthDelta":-8000}]"#,
    );
    dump(&doc, pi, ci, "B resizeTableCells -8000x2");
    // C: setTableColumnWidths
    let r = doc.set_table_column_widths(0, pi, ci, "[10000,10000]");
    println!(
        "  setTableColumnWidths resp={:?}",
        r.map(|x| x.chars().take(60).collect::<String>())
    );

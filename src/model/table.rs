...
impl Table {
    ...
    /// 어긋낸 경계를 정렬선으로 복원 — `offset_cell_boundary` 의 역연산(래퍼: 트랜잭션, 셀 수 보존).
    ///
    /// 절차는 `restore_cell_boundary_core`([11-b] 컨텍스트 R 에 p := pos(A) 강제 → join_to_line + collapse_dead_lines).
    /// 어긋난 적 없는 셀에 대한 오발 호출이 격자를 키우던 사고(2026-08-16, 키보드 연속 +283HU/회)는
    /// 결과 불변식·델타 관문(check_invariants/check_deltas) 위반 시 롤백 + 거부로 막는다.
